...
bb_1_p <- lm.mcmc(y,X, model.prior = beta_binomial_complexity.prior(1, 0, 1, 1), 
                  burnin.iterations = 0, mcmc.iterations = 1e7)
md_theta_1 <- lm.mcmc(y,X, model.prior = 
                        matryoshka_doll.prior(1/expm1(1), 'descendants'),
                      burnin.iterations = 0, mcmc.iterations = 1e7)
@

<<echo=FALSE>>=
xlim = range(c(bb_1_p$size, md_theta_1$size))
@
...
<<label=boxplot_md_theta_1, echo=FALSE, eval=FALSE>>=
with(md_theta_1, boxplot(postprobs~factor(size, levels=c(0:p)+1),
                         ylim=c(0,1), xlim=xlim,
                         main=expression(MD(theta==1)),
                         xlab='Model size', ylab='Posterior probability'))
@
